...
def processar_arquivo(arquivo_path: str, tipo: str, botao: int) -> str:
    ...
    # Se for .bin, copiar direto sem tentar processar
    if arquivo_path.endswith('.bin'):
        log.info(f"📁 Arquivo .bin detectado, copiando sem processamento...")
        output_filename = f"{tipo}_botao_{botao}.bin"
        output_path = os.path.join(UPLOADS_DIR, output_filename)
        try:
            shutil.copy(arquivo_path, output_path)
            log.info(f"✅ Arquivo .bin copiado: {output_filename}")
            return output_path
        except Exception as e:
